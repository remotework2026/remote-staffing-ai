...
const FROM_EMAIL = process.env.FROM_EMAIL || "digitaltrading76@gmail.com";
...
try {
  if (
    ...
    console.log("Google Sheets connected: NO - missing env variables");
  }
} catch (err) {
...
async function readSheetRows(tabName) {
  ...
  try {
    ...
    console.log(
      `Google Sheets read error for ${tabName}:`,
      err.response?.data || err.message
    );
...
app.get('/public-content', async (req, res) => {
  ...
  if (articles.length === 0) {
    articles = [
      {
        title: "Why Businesses Hire Filipino Virtual Assistants",
        category: "Filipino VA Hiring",
        content: "Filipino virtual assistants help businesses reduce costs while getting reliable support for admin work, email handling, scheduling, research, ecommerce tasks, and customer service.",
  ...
  if (jobs.length === 0) {
    jobs = [
      {
        jobTitle: "Filipino General Virtual Assistant",
        jobType: "Remote",
        experience: "Beginner Friendly",
        salaryRange: "Negotiable",
        description: "Support international businesses with admin tasks, scheduling, email management, and research.",
        status: "Active"
      },
      {
        jobTitle: "Filipino Customer Support VA",
        jobType: "Remote",
        experience: "Customer service experience preferred",
        salaryRange: "Negotiable",
        description: "Handle customer inquiries, chat support, order updates, and email communication.",
        status: "Active"
      }
    ];
  }
...
app.get('/client-leads', requireAuth, (req, res) => {
  res.json(readJSON(CLIENT_LEADS_FILE));
});

app.get('/applicant-leads', requireAuth, (req, res) => {
  res.json(readJSON(APPLICANT_LEADS_FILE));
});
...
app.post('/run-match-notifications', requireAuth, async (req, res) => {
  ...
  for (const client of clients) {
    ...
    await sendMail(
      client.email,
      "Possible Filipino Remote Staff Match Found",
`Hi ${client.name},

We found a possible Filipino VA match for your requested role: ${client.role}
...
Remote Staff Agency`
    ...
    await sendMail(
      bestApplicant.email,
      "Possible Filipino VA Job Opportunity Match",
